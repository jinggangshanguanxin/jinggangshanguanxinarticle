...
用法：python .workbuddy/checks/browser-check.py [关键词] [图表关键词]
"""
...
def main():
    ...
    dump = ROOT / '.workbuddy' / 'tmp' / 'dump.html'
    dump.parent.mkdir(parents=True, exist_ok=True)

    url = (f'http://127.0.0.1:{port}/_selftest.html'
           f'?kw={quote(KW)}&kw3={quote(KW3)}')
    cmd = [
    ...
    try:
        ...
        notes = parse_notes(dump.read_text(encoding='utf-8', errors='replace'))
    finally:
        httpd.shutdown()
